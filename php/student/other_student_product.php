...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {       // 处理表单
	...
	if(empty($comment))
		...
		$res = get_student_product_info_id($pdo, $_GET['student_id'], $homework_id); 
					foreach ($res as $row) {
						$product_id = $row['id'];
					}
...
<body>
 ...
 foreach ($files as $row_file) {
	...
	<div class="cd-articles">
		<article>
			<header>
				<h1><?php $res = sel_student_info_id($pdo, $_GET['student_id']);
				foreach ($res as $row) {
			...
			<p>
				<?php $res = get_student_product_info_id($pdo, $_GET['student_id'], $homework_id); 
					foreach ($res as $row) {
		...
		<div class="student_comments">
			<ul>
				<?php $res_product = get_student_product_info_id($pdo, $_GET['student_id'], $homework_id); 
...
<!--侧边栏-->	
		<div class="other_student_product" style="height:auto;width:auto;background-color: #fff;position: absolute;top: 80px;right: -150px;word-break: break-all;color: white;min-height: 100px;min-width: 300px;max-width: 500px;">
			...
			<div class="other_student_style">
				<ul>
					<?php 
					 	$res = get_other_product_info($pdo, $homework_id, $_GET['student_id']);
					 	foreach ($res as $row) {
					 	 ?>
					<li>
